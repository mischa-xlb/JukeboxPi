...
def find_device_id(device_name):
    print("[DEBUG] === Starting find_device_id ===")
    print(f"[DEBUG] Searching for device: '{device_name}'")
    
    try:
        print("[DEBUG] Calling sp.devices()...")
        ...
        def timeout_handler(signum, frame):
            raise TimeoutError("sp.devices() call timed out")
        ...
        try:
            ...
            print(f"[DEBUG] sp.devices() returned successfully")
        except TimeoutError as e:
            print(f"[DEBUG] TIMEOUT: {e}")
            print("[DEBUG] This usually means:")
            print("  1. Your Spotify token has expired")
            print("  2. Network connectivity issue")
            print("  3. Spotify API is not responding")
            return None
        
        print(f"[DEBUG] Type of devices: {type(devices)}")
        print(f"[DEBUG] devices content: {devices}")
    except Exception as e:
        print(f"[DEBUG] ERROR calling sp.devices(): {e}")
        import traceback
        traceback.print_exc()
        return None
    
    print("[DEBUG] All available devices:")
    if 'devices' in devices:
        print(f"[DEBUG] Found {len(devices['devices'])} devices")
        for i, device in enumerate(devices['devices']):
            print(f"[DEBUG] Device {i}:")
            print(f"  - Name: '{device['name']}'")
            print(f"  - Type: {device['type']}")
            print(f"  - Active: {device['is_active']}")
            print(f"  - ID: {device['id']}")
    else:
        print("[DEBUG] ERROR: 'devices' key not found in response")
        return None
    
    print(f"[DEBUG] Now searching for match with '{device_name}'...")
    
    # Search for matching device
    for i, device in enumerate(devices['devices']):
        print(f"[DEBUG] Checking device {i}: '{device['name']}'")
        print(f"[DEBUG] Comparing '{device['name'].lower()}' == '{device_name.lower()}'")
    ...
    print(f"[DEBUG] === Loop completed - no match found ===")

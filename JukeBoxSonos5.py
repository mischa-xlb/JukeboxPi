...
def announce_selection(number, title):
    ...
    if config['announce_selections']:
        ...
        result = os.system(f'espeak "{message}" 2>&1')
        print(f"[DEBUG] espeak return code: {result}")
        
        if result != 0:
            print("[WARNING] espeak may not be installed or configured correctly")
            print("[INFO] Install with: sudo apt-get install espeak")
